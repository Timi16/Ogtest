...
async function chat(b: any, provider: string, messages: Msg[]) {
...
(async () => {
  ...
  const selectPrompt: Msg[] = [
    {
      role: "system",
      content:
        "You select the most relevant snippets for a question. Reply with a JSON array of indices only, e.g. [1,2]. No text.",
    },
    {
      role: "user",
  ...
  const answerPrompt: Msg[] = [
    {
      role: "system",
      content:
        "You are a precise assistant. Use ONLY the given context. If not in context, say you don't know.",
    },
