...
const CHAT_PROVIDER = process.env.CHAT_PROVIDER!; // e.g. 0xf07240Efa67755B5311bc75784a061eDB47165Dd
...
async function chat(b: any, provider: string, messages: {role:"system"|"user"|"assistant", content:string}[]) {
  const { endpoint, model } = await b.inference.getServiceMetadata(provider);
  await b.inference.acknowledgeProviderSigner(provider);
  const billText = messages.map(m=>`${m.role}: ${m.content}`).join("\n").slice(0,4000);
...
function label(chunks: string[]) {
  return chunks.map((t,i)=>`[${i}] ${t}`).join("\n");
}

(async () => {
  if (!RPC_URL || !PRIVATE_KEY || !CHAT_PROVIDER) throw new Error("Set RPC_URL, PRIVATE_KEY, CHAT_PROVIDER in .env");
  ...
  // Step 1: LLM selects relevant chunk indices (JSON array only)
  const selectPrompt = [
    { role: "system", content: "You select the most relevant snippets for a question. Reply with a JSON array of indices only, e.g. [1,2]. No text." },
    { role: "user", content: `Question: ${question}\nSnippets:\n${label(docs)}` }
  ];
  let raw = await chat(b, CHAT_PROVIDER, selectPrompt);

  let idx: number[] = [];
  try { idx = JSON.parse(raw); } catch { idx = [0,1]; }
  if (!Array.isArray(idx) || !idx.every(n=>Number.isInteger(n) && n>=0 && n<docs.length)) idx = [0,1];

  const context = idx.map(i=>docs[i]).join("\n");

  // Step 2: Final answer grounded on selected snippets
  const answerPrompt = [
    { role: "system", content: "You are a precise assistant. Use ONLY the given context. If not in context, say you don't know." },
